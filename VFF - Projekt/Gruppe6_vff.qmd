...
pacman::p_load(
  ...
  corrplot,      # Bruges til at visualisere korrelationsmatricer. Gør det let at identificere stærke sammenhænge mellem variabler. (ikke sikker på vi har haft med bjarn)
...
excel_guld$dato[is.na(excel_guld$dato)] <- as.Date(as.numeric(excel_guld$Dato[is.na(excel_guld$dato)]), 
                                                   origin = "1899-12-30") # startdato i xcel 
...
excel_guld <- excel_guld |> 
  ...
  mutate(guld_menu_stk = as.integer(as.numeric(Guld_menu_stk))) |> # tager et helt tal istedet for decimaler
...
excel_guld <- excel_guld |> 
  mutate(
    guld_menu_stk = ifelse(
      is.na(guld_menu_stk), 
      round(mean(guld_menu_stk, na.rm = TRUE)),  # Erstat med afrundet gennemsnit.# true køre men springer over na og dividere med antal 97 round står for ikke kommatal.
      guld_menu_stk
    )
  )
...
valider_tabel <- function(tabel) {
  nrow(tabel) > 0 &&  # Tjekker om tabellen ikke er tom nrow
    all(c("X3", "X4", "X5") %in% names(tabel)) &&  # Tjekker om kolonnerne findes  x3kampe x4resultat x5 tilskuere 
    any(tabel$X3 != "" | tabel$X4 != "" | tabel$X5 != "")  # Tjekker om der er data i mindst én af kolonnerne (! betyder den må ikke være tom)
}
...
superstats <- purrr::map_dfr(2013:2023, hent_superstats_data) |> #når de får en ny kan man lave 23 om 24 funktionen linje 683
  dplyr::select(-X1, -X6, -X7, -X8) |>  # Fjern kolonner, der ikke er nødvendige (fjerne eksempelvis dommer unødvendige variabler)
  filter(!(X3 == "" & X4 == "" & X5 == ""))  # Fjern rækker uden værdier i relevante kolonner - hvis der ikke er noget i, så fjerner jeg dem. (måske gentagelse)

# ------------------------------------------------------------------------------
# Filtrér og rengør data for VFF-hjemmekampe
# ------------------------------------------------------------------------------
superstats_vff <- superstats |> 
  filter(str_detect(X3, "VFF")) |>  # Behold kun rækker, hvor kolonnen 'X3' indeholder "VFF" (dvs. VFF-kampe) hvis der ikke står vff er vi ligeglad
  rename(
  ...
  mutate(
    ...
    årstal = ifelse(                         # Bestem årstal baseret på måneden
      måned >= 7,                            # Hvis måneden er juli eller senere sæsonen skiller i juli 
      as.numeric(substr(sæson, 1, 4)),       # Brug første år i sæsonen
      as.numeric(substr(sæson, 6, 9))        # Ellers brug andet år i sæsonen
    ),
    dato = as.Date(paste(årstal, format(dato, "%m-%d"), sep = "-")),  # Kombinér årstal, måned og dag til en komplet dato sep. skal være bindestreg
...
hjemmekampe_datoer <- format(superstats_vff$dato, "%Y-%m-%d") #du kan viwe hjemmekampe.
# ------------------------------------------------------------------------------
...
station_id <- "stationId=06060"  # Identifikator for vejrstationen karup
...
dmi_variabler <- purrr::map_dfr(names(dmi_data), function(dato) { # map_dfr er hvor den bygger fatafamen udfra dato 
...
#____________________________________________________________________________818
## Omskrevet til en bedre og mere simpel kode uden 'Observrtionstidspunkt':
...
# ------------------------------------------------------------------------------
# Vi fjerner kolonner, der indeholder minimum, maksimum, eller midlingsværdier, da de ikke er nødvendige. og giver en dårlige RMSE 220 - så 13 dmi variabler nu 
...
dmi_variabler <- dmi_variabler |> 
  mutate(
    nedbør = if_else(
      ...
      1,  # Nedbør til stede når et af overstående tal er i nedbøre 
      0   # Ingen nedbør
    )
  ) |> 
...
joins_samlet_variabler <- joins_samlet_variabler |> 
  ...
  relocate(modstander, .after = kamp) |>  # Flyt 'modstander' tættere på 'kamp'. i datasættet 
...
joins_samlet_variabler <- joins_samlet_variabler |> 
  ...
  mutate(
    ...
    guld_menuer_sidste_møde = ifelse(
      ...
      slide_dbl(guld_menu_stk, mean, .before = 2, .complete = TRUE),  #når den finder na, så går den videre til at finde medianen for hele guld menu stk mod f.eks randers
...
# men lasso og rigs regression fjerne dem automatisk 

# ------------------------------------------------------------------------------
# Lav en lineær model med numeriske variabler og beregn VIF
# ------------------------------------------------------------------------------
# Byg en lineær model for at undersøge multikollinearitet med Variance Inflation Factor (VIF). altså om der er stor usikkerhed
...
set.seed(42) # man kan gemkøre modellen og få det samme resultat alle sammen 42 var det nørderne bruger (ligemeget hvad men samme koifficent og RMSE resultat) vil den tilfældige talgenerator starte fra samme punkt hver gang, hvilket gør det muligt at få de samme tilfældige resultater hver gang, når du kører koden.sikre, at dine eksperimenter og resultater kan reproduceres af andre

train <- sample(1:nrow(joins_samlet_model), nrow(joins_samlet_model) * 2/3)  # 2/3 af data som træning.
#får tilstrækkeligt med data til at lære på, samtidig med at der er nok data til at evaluere modellens præstation på usete data
test <- (-train)  # Resten som testdata.Testdatasættet bruges til at vurdere, hvordan modellen generaliserer på data, den ikke har set før
...
bestlam_ridge <- cv.ridge$lambda.min  # Optimal lambda. lamba bruges til at kontrollere styrken af regularisering i modeller som Ridge og Lasso. Regularisering hjælper med at reducere overfitting ved at tilføje en straf til modellens koefficienter.
...
cv.out <- cv.glmnet(x[train, ], y[train], alpha = 1, lambda = grid, nfolds = 10)#hvor datasættet opdeles i 10 lige store "folds", og modellen trænes og evalueres 10 gange.
...
# Beregn RMSE for hver model 133 forkert både positvt og ne meget. tillækker dem der har skudt forkert mere værdi. 
...
# Beregn MAE (Mean Absolute Error) for hver model -antal mennesker i snit den skyder forket Det måler den gennemsnitlige absolutte forskel mellem de faktiske værdier og de forudsigte værdier. tager forbehold for dem der er skudt meget ved siden af.
baseline_mae <- mean(abs(y.test - predict(glm.fit, joins_samlet_model[test, ])))  # MAE for baseline-modellen. 139 enheder forskel
ridge_mae <- mean(abs(ridge_pred - y.test))  # MAE for Ridge Regression. 121 enheder forskel
lasso_mae <- mean(abs(as.numeric(lasso.pred) - y.test))  # MAE for Lasso Regression. 90 enheder i forskel
...
# Vi bemærker, at residualerne generelt er spredt omkring 0-linjen, 
# hvilket indikerer, at modellen har en rimelig god performance uden systematiske fejl.
# Dog ser vi, at der er enkelte større residualer, især for observationer med høje værdier. 
# Dette kan tyde på, at modellen ikke forudsiger disse observationer korrekt. 
# Vi vurderer, at dette muligvis skyldes, at vigtige variabler mangler i modellen, 
# eller at der er ikke-lineære sammenhænge, som modellen ikke fanger.
# For at forbedre modellen vil vi foreslå en yderligere undersøgelse af dataene, 
# inkludering af nye variabler samt en mulig transformation af eksisterende variabler. 
# Vi kan også overveje at teste alternative modeller for at opnå bedre præcision.
...
# Dog bliver det nu tydeligere, at der er flere outliers i den positive ende, 
# især omkring residualværdier tæt på 400. Dette kan være observationer, som modellen har svært ved at forudsige korrekt.
...
# Evaluering af plot over faktiske vs. forudsagte værdier:
# Dette plot sammenligner de faktiske værdier med de forudsagte værdier fra Lasso-modellen.
# Den røde linje repræsenterer den ideelle linje, hvor forudsigelserne nøjagtigt matcher de faktiske værdier (y = x).
# Punktfordelingen viser, at mange punkter ligger tæt på linjen, hvilket indikerer, at modellen generelt præsterer godt.
# Dog bemærker vi enkelte punkter, der afviger betydeligt fra linjen, især i den højere ende af værdierne. 
# Dette indikerer, at modellen har sværere ved at forudsige ekstreme værdier korrekt.
# For at forbedre modellen kunne vi overveje at inkludere yderligere variabler.

# Samlet konklusion:
# Gennem vores analyser af residualplottet, histogrammet over residualerne og plottet for faktiske vs. forudsagte værdier, 
# konkluderer vi, at Lasso-modellen generelt præsterer godt med en fornuftig fordeling af residualerne omkring nul 
...
# For at forbedre modellen foreslår vi at:
...
# Vi vurderer samlet, at Lasso-modellen er en god start, men der er plads til forbedring for at sikre mere præcise og robuste forudsigelser.
```